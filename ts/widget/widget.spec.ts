...
describe('W + S Test', () => {
    describe('parent, scene', () => {
        ...
        test('W -> S', () => {
            ...
            s.addChild(w);
            expect(s.all).toEqual([w]);
            expect(s.children).toEqual([w]);
        ...
        test('W(s)', () => {
            const s = new Scene('scene');
            const w = new WIDGET.Widget({ scene: s });
        ...
        test('S(), P(S), W(P)', () => {
            const s = new Scene('scene');
            const p = new WIDGET.Widget({ scene: s, id: 'P' });
            const w = new WIDGET.Widget({ parent: p, id: 'W' });

            expect(IDS(s.all)).toEqual(['P', 'W']);
            expect(IDS(s.children)).toEqual(['P']);
        ...
        test('P(), W(P), S+W', () => {
            ...
            expect(IDS(s.all)).toEqual(['W']);
            expect(IDS(s.children)).toEqual(['W']);
        ...
        test('P(), W(P), S+W, S+P', () => {
            ...
            expect(IDS(s.all)).toEqual(['W', 'P']);
            expect(IDS(s.children)).toEqual(['W', 'P']);
            expect(IDS(p.children)).toEqual([]);
            expect(IDS(w.children)).toEqual([]);
