...
export function configure(opts:Partial<RandomConfig>) {
    if (opts.make) {
        if (typeof opts.make !== 'function') throw new Error('Random make parameter must be a function.');
        if (typeof opts.make(12345) !== 'function') throw new Error('Random make function must accept a numeric seed and return a random function.');
...
export class Random {
    ...
    int(max:number=0) { return this.number(max); }
