...
describe('buffer', () => {
    describe('DataBuffer', () => {
        ...
        test('constructor', () => {
            ...
            expect(b.get(0, 0)).toEqual({ glyph: 0, bg: 0, fg: 0 });
        });

        test('draw', () => {
            const b = new Buffer.DataBuffer(10, 8);
            b.draw(1, 2, 3, 0xf00, 0x0f0);
            expect(b.get(1, 2)).toEqual({ glyph: 3, fg: 0xf00, bg: 0x0f0 });

            b.draw(1, 2);
            expect(b.get(1, 2)).toEqual({ glyph: 3, fg: 0xf00, bg: 0x0f0 });

            b.draw(2, 3, 'A', colors.white, colors.gray);
            expect(b.get(2, 3)).toEqual({ glyph: 65, fg: 0xfff, bg: 0x888 });
        ...
        test('drawSprite', () => {
            ...
            expect(b.get(1, 2)).toEqual({ glyph: 66, fg: 0xf00, bg: 0x0f0 });

            b.drawSprite(1, 2, { fg: 0x0f0 });
            expect(b.get(1, 2)).toEqual({ glyph: 66, fg: 0x0f0, bg: 0x0f0 });
        });
        ...
        test('copy', () => {
            ...
            a.copy(b);
            expect(a.get(1, 2)).toEqual({ glyph: 65, fg: 0x321, bg: 0x987 });
        });
        ...
        test('drawText', () => {
            ...
            expect(b.get(1, 0)).toEqual({
                glyph: 'e'.charCodeAt(0),
                fg: 0xfff,
                bg: 0,
            });
            expect(b.get(2, 0)).toEqual({
                glyph: 's'.charCodeAt(0),
                fg: 0xfff,
                bg: 0,
            });
            expect(b.get(3, 0)).toEqual({
                glyph: 't'.charCodeAt(0),
                fg: 0xfff,
                bg: 0,
            });
        ...
        test('wrapText', () => {
            ...
            expect(b.get(0, 2)).toEqual({
                glyph: 't'.charCodeAt(0),
                fg: 0xfff,
                bg: 0,
            });
            expect(b.get(0, 3)).toEqual({
                glyph: 's'.charCodeAt(0),
                fg: 0xfff,
                bg: 0,
            });
            ...
            expect(b.get(9, 7)).toEqual({ glyph: 0, fg: 0, bg: 0x00f });
            expect(b.get(9, 8)).toEqual({ glyph: 0, fg: 0, bg: 0x00f });
        });
        ...
        test('fillRect', () => {
            ...
            expect(b.get(2, 3)).toEqual({ glyph: 65, fg: 0x0f0, bg: 0x00f });
        ...
        test('blackOutRect', () => {
            ...
            expect(b.get(2, 2)).toEqual({ glyph: 0, fg: 0, bg: 0 });
            expect(b.get(3, 3)).toEqual({ glyph: 65, fg: 0xf00, bg: 0x00f });
        ...
        test('highlight', () => {
            ...
            expect(b.get(1, 1)).toEqual({ glyph: 65, fg: 0xf88, bg: 0x88f });
