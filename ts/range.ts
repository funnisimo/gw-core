...
export class Range {
	...
	constructor(lower:number|Range|number[], upper=0, clumps=1, rng?:Random) {
...
export function make(config:Range|string|number[]|null, rng?:Random) {
    ...
    if (typeof config == 'function') throw new Error('Custom range functions not supported - extend Range');
  
    if (config === undefined || config === null) return new Range(0, 0, 0, rng);
    if (typeof config == 'number') return new Range(config, config, 1, rng);

    // @ts-ignore
    if (config === true || config === false) throw new Error('Invalid random config: ' + config);
